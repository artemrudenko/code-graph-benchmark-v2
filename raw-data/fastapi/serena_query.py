...
async def main():
    ...
    async with streamablehttp_client(url) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            t0 = time.time()
            r1 = await session.call_tool("find_symbol", {"name_path": "solve_dependencies", "relative_path": "dependencies/utils.py", "include_body": True})
            t1 = time.time() - t0
            results['find_symbol'] = {'time_s': t1, 'content': [c.text if hasattr(c,'text') else str(c) for c in r1.content]}

            t0 = time.time()
            r2 = await session.call_tool("find_referencing_symbols", {"name_path": "solve_dependencies", "relative_path": "dependencies/utils.py"})
            t2 = time.time() - t0
